...
// TestAliasKindIsDirectoryScoped verifies that a "# gazelle:alias_kind"
// directive set in one package does not leak into unrelated packages
// elsewhere in the repository. The repo root declares an alias_kind of its
// own so that its AliasMap is allocated and inherited by both "leaf" and
// "other"; "leaf" then adds a second alias_kind, and "other" is an unrelated
// sibling package that never opts in and must be unaffected by "leaf"'s
// directive.
func TestAliasKindIsDirectoryScoped(t *testing.T) {
	before := []testtools.FileSpec{
		{
			Path: "WORKSPACE",
		},
		{
			Path: "BUILD.bazel",
			Content: `
# gazelle:alias_kind repo_wide_macro go_binary
`,
		},
		{
			Path: "leaf/BUILD.bazel",
			Content: `
# gazelle:alias_kind my_custom_macro go_library
`,
		},
		{
			Path: "other/BUILD.bazel",
			Content: `
# gazelle:prefix example.com/other
# gazelle:go_naming_convention go_default_library
load("//custom:def.bzl", "my_custom_macro")
...
import (
	_ "example.com/other/bar"
)
`,
		},
	}
	// "other/BUILD.bazel" must come out byte-for-byte unchanged: it never
	// declared alias_kind, so its hand-written my_custom_macro rule is not a
	// recognized or aliased kind there, and gazelle must leave it alone.
	after := before

	dir, cleanup := testtools.CreateFiles(t, before)
	t.Cleanup(cleanup)
	args := []string{"-external=vendored", "-index=false"}
	if err := runGazelle(dir, args); err != nil {
		t.Fatal(err)
	}

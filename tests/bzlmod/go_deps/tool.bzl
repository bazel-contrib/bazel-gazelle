...
including a Go module in a subdirectory of that Bazel module (dep_tool2
in tool_dep/v2). A tool at the root of a module with a major version
suffix (example.com/tool_dep/v2) is invoked by the name before the
suffix (tool_dep), but Gazelle names its go_binary after the last path
element (v2). For Bazel modules, the label uses the canonical repo
name of the go.mod file's label, which is empty for the real root
module; the test harness uses the module name instead.
"""

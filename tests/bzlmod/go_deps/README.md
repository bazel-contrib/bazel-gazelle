...
1. Create a scratch directory. For each Bazel module in the test, create a
   subdirectory (named after the module) containing a `MODULE.bazel` file:
...
1. Unpack the test into a scratch directory. The output directory must be
   empty unless you pass `-f` to remove it first:

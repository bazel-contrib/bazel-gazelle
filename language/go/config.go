...
type goConfig struct {
	...
	// gcGoopts, gcLinkopts, copts, cppopts, cxxopts, and clinkopts are lists of
	// Go compiler / C compiler / linker flags applied as the corresponding
	// attributes of generated go_library, go_binary, and go_test rules. They are
	// set with the go_gc_goopts, go_gc_linkopts, go_copts, go_cppopts,
	// go_cxxopts, and go_clinkopts directives respectively, and are primarily
	// useful for pushing flags onto external dependencies generated by go_deps,
	// which have no hand-written build files to edit.
	//
	// The C flags (copts, cppopts, cxxopts, clinkopts) only apply to cgo targets
	// and are merged with the flags Gazelle derives from #cgo comments. The
	// linker flags (gcLinkopts, clinkopts) only apply to rules that link a
	// binary, i.e. go_binary and go_test.
	gcGoopts   []string
	gcLinkopts []string
	copts      []string
	cppopts    []string
...
func (gc *goConfig) clone() *goConfig {
	...
	gcCopy.gcGoopts = gc.gcGoopts[:len(gc.gcGoopts):len(gc.gcGoopts)]
	gcCopy.gcLinkopts = gc.gcLinkopts[:len(gc.gcLinkopts):len(gc.gcLinkopts)]
	gcCopy.copts = gc.copts[:len(gc.copts):len(gc.copts)]
	gcCopy.cppopts = gc.cppopts[:len(gc.cppopts):len(gc.cppopts)]
	gcCopy.cxxopts = gc.cxxopts[:len(gc.cxxopts):len(gc.cxxopts)]
	gcCopy.clinkopts = gc.clinkopts[:len(gc.clinkopts):len(gc.clinkopts)]
	return &gcCopy
}
...
Update io_bazel_rules_go to a newer version in your WORKSPACE file.`
	...
	gl.configureDirState(c, gc, rel, f)
}

...
**Default:** n/a<br>
Sets the `gc_goopts` attribute (Go compiler flags) on generated `go_library`, `go_binary`, and `go_test` rules. Flags may be separated by whitespace and/or commas, so `-N -l` and `-N,-l` are equivalent. Omit the value to reset the directive.

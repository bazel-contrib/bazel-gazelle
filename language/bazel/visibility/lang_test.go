...
func Test_PackageFeatures_NewDirective(t *testing.T) {
	testFeature := "layering_check"
	cfg := config.New()
	file, err := rule.LoadData("path", "pkg", []byte(fmt.Sprintf("# gazelle:default_features %s", testFeature)))
	if err != nil {
		t.Fatal("expected nil")
	}
	...
	features := res.Gen[0].AttrStrings("features")
	if len(features) != 1 {
		t.Fatalf("expected features array of length 1, got %d", len(features))
	}
	if testFeature != features[0] {
		t.Fatalf("expected feature to match '%s', got '%s'", testFeature, features[0])
	}
}

func Test_PackageFeatures_MultipleFeatures(t *testing.T) {
	testFeature1 := "layering_check"
	testFeature2 := "-parse_headers"
	cfg := config.New()
	file, err := rule.LoadData("path", "pkg", []byte(fmt.Sprintf("# gazelle:default_features %s,%s", testFeature1, testFeature2)))
	if err != nil {
		t.Fatal("expected nil")
	}
	...
	features := res.Gen[0].AttrStrings("features")
	if len(features) != 2 {
		t.Fatalf("expected features array of length 2, got %d", len(features))
	}
	if testFeature1 != features[0] {
		t.Fatalf("expected first feature to match '%s', got '%s'", testFeature1, features[0])
	}
	if testFeature2 != features[1] {
		t.Fatalf("expected second feature to match '%s', got '%s'", testFeature2, features[1])
	}
}

func Test_PackageFeatures_WithVisibilityDirective(t *testing.T) {
	testVis := "//src:__subpackages__"
	testFeature := "layering_check"
	cfg := config.New()
	file, err := rule.LoadData("path", "pkg", []byte(fmt.Sprintf(`
# gazelle:default_visibility %s
# gazelle:default_features %s
`, testVis, testFeature)))
	if err != nil {
		t.Fatal("expected nil")
	}
	...
	visibility := res.Gen[0].AttrStrings("default_visibility")
	if len(visibility) != 1 {
		t.Fatalf("expected visibility array of length 1, got %d", len(visibility))
	}
	if testVis != visibility[0] {
		t.Fatalf("expected visibility to match '%s', got '%s'", testVis, visibility[0])
	}
...
func Test_PackageFeatures_PreservesExistingVisibility(t *testing.T) {
	testFeature := "layering_check"
	existingVis := "//visibility:public"
	cfg := config.New()

	configFile, err := rule.LoadData("path", "pkg", []byte(fmt.Sprintf("# gazelle:default_features %s", testFeature)))
	if err != nil {
		t.Fatal("expected nil")
	}

	ext := visibility.NewLanguage()
	ext.Configure(cfg, "rel", configFile)

	buildFile, err := rule.LoadData("path/file", "pkg", []byte(fmt.Sprintf(`
package(default_visibility = ["%s"])
`, existingVis)))
	if err != nil {
		t.Fatal("expected nil")
	}
	...
	if existingVis != visibility[0] {
		t.Fatalf("expected visibility to be preserved as '%s', got '%s'", existingVis, visibility[0])
	}
}

func Test_PackageVisibility_PreservesExistingFeatures(t *testing.T) {
	existingFeature := "layering_check"
	testVis := "//visibility:public"
	cfg := config.New()

	configFile, err := rule.LoadData("path", "pkg", []byte(fmt.Sprintf("# gazelle:default_visibility %s", testVis)))
	if err != nil {
		t.Fatal("expected nil")
	}
...
`, existingFeature)))
	...
	if existingFeature != features[0] {
		t.Fatalf("expected feature to match '%s', got '%s'", existingFeature, features[0])
	}
	...
	if testVis != visibility[0] {
		t.Fatalf("expected visibility to be preserved as '%s', got '%s'", testVis, visibility[0])
	}
}
